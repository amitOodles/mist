...
function _load_util() {
    return _util = _interopRequireWildcard(require("util"));
}
...
class Packager {
    ...
    build() {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            ...
            const fileOrPackageConfig = yield (0, (_readPackageJson || _load_readPackageJson()).loadConfig)(_this.projectDir);
            const config = fileOrPackageConfig == null ? configFromOptions : (0, (_deepAssign || _load_deepAssign()).deepAssign)(fileOrPackageConfig, configFromOptions);
            ...
            _this.appDir = yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).computeDefaultAppDirectory)(_this.projectDir, (0, (_electronBuilderUtil || _load_electronBuilderUtil()).use)(config.directories, function (it) {
                return it.app;
            }));
            _this.isTwoPackageJsonProjectLayoutUsed = _this.appDir !== _this.projectDir;
            if (_this.isTwoPackageJsonProjectLayoutUsed) {}
            const devPackageFile = _path.join(_this.projectDir, "package.json");
            ...
            _this.electronVersion = yield (0, (_readPackageJson || _load_readPackageJson()).getElectronVersion)(_this.config, _this.projectDir);
    ...
    doBuild(cleanupTasks) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            const distTasks = [];
            const outDir = _path.resolve(_this2.projectDir, (0, (_electronBuilderUtil || _load_electronBuilderUtil()).use)(_this2.config.directories, function (it) {
                return it.output;
            }) || "dist");
    ...
    checkMetadata(appPackageFile, devAppPackageFile) {
        ...
        if (appMetadata !== this.devMetadata) {
            ...
            if (appMetadata.build != null) {
                throw new Error((_util || _load_util()).format((_errorMessages || _load_errorMessages()).buildInAppSpecified, appPackageFile, devAppPackageFile));
            }
        }
        const build = this.config;
        if (build == null) {
            throw new Error((_util || _load_util()).format((_errorMessages || _load_errorMessages()).buildIsMissed, devAppPackageFile));
        } else {
            ...
            if (build.name != null) {
                throw new Error((_util || _load_util()).format((_errorMessages || _load_errorMessages()).nameInBuildSpecified, appPackageFile));
            }
            if (build.prune != null) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            ...
            if (options.nodeGypRebuild === true) {
                ...
                yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)(process.platform === "win32" ? "node-gyp.cmd" : "node-gyp", ["rebuild"], {
                    env: (0, (_yarn || _load_yarn()).getGypEnv)(_this3.electronVersion, platform.nodeName, (_electronBuilderCore || _load_electronBuilderCore()).Arch[arch], true)
                });
            }
            ...
            if (beforeBuild != null) {
                const performDependenciesInstallOrRebuild = yield beforeBuild({
                    appDir: _this3.appDir,
                    electronVersion: _this3.electronVersion,
                    platform,
            ...
            if (options.npmSkipBuildFromSource !== true && platform.nodeName !== process.platform) {
                ...
                yield (0, (_yarn || _load_yarn()).installOrRebuild)(options, _this3.appDir, _this3.electronVersion, platform.nodeName, (_electronBuilderCore || _load_electronBuilderCore()).Arch[arch]);
            }
        })();
    }
    afterPack(context) {
